...
def main():
    ...
    # 时间守卫：仅在北京时间 19:00~22:59 之间推送
    # GitHub cron UTC 12:00(BJ 20:00) 正常到达或延迟1-2小时都覆盖
    bj_hour = beijing_now.hour
    if bj_hour < 19 or bj_hour >= 23:
        logger.warning(f"当前北京时间 {bj_hour}:00 不在推送窗口(19:00~22:59)，跳过推送")
        # 仍写入状态文件记录
